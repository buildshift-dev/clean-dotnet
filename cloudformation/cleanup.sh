...
# Clean Architecture .NET - Cleanup Script
...
echo_warning "This will delete all .NET Clean Architecture demo resources for environment: $ENVIRONMENT in region: $REGION"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    REPO_NAME="clean-dotnet-$ENVIRONMENT-$APP_NAME-api"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    docker rmi clean-dotnet-$APP_NAME:latest 2>/dev/null || true
...
echo_info "All .NET Clean Architecture demo resources for environment '$ENVIRONMENT' have been removed."
echo ""
echo_info "Stack names that were deleted:"
echo_info "- clean-dotnet-$ENVIRONMENT-$APP_NAME-ecs"
echo_info "- clean-dotnet-$ENVIRONMENT-$APP_NAME-vpc-alb" 
echo_info "- clean-dotnet-$ENVIRONMENT-$APP_NAME-ecr (if selected)"